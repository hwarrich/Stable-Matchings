...
public class SMInstance {
    ...
    public Matching getMatching () {
		Matching myMatch = new Matching(this);
		for(Agent res: residents){
			if(res != null && res.getCurMatch() != null){
				myMatch.addPair(new Pair<Agent> (res, res.getCurMatch()));
			}
		}
		return myMatch;
    }
    ...
    public void setMatching (Matching matching) {
		for(Agent res: residents){
			res.setCurMatch(matching.getMatch(res));
		}
		for(Agent hos: hospitals){
			hos.setCurMatch(matching.getMatch(hos));
		}
    }
    ...
    public Pair<Agent> getBlockingPair () {
		for(Agent res: residents) {
			ArrayList<Agent> resPrefList = res.getPrefList();
			for(int i =0; i < resPrefList.size(); i++){
				Agent hos = resPrefList.get(i);
				if(res.prefers(hos, res.getCurMatch()) && hos.prefers(res, hos.getCurMatch())){
					return new Pair<Agent>(res, hos);
				}
			}
		}
		return null;
